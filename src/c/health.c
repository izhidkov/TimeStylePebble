...
static inline HealthValue get_health_value_sum_today(HealthMetric metric) {
  return is_health_metric_accessible(metric) ? health_service_sum_today(metric)
                                             : 0;
}
...
static void health_event_handler(HealthEventType event, void *context) {
  if (event == HealthEventSignificantUpdate) {
    ...
    s_sleeping =
        (mask & HealthActivitySleep) || (mask & HealthActivityRestfulSleep);
...
bool Health_isUserSleeping(void) { return s_sleeping; }

HealthValue Health_getSleepSeconds(void) { return s_sleep_seconds; }

HealthValue Health_getSteps(void) { return s_steps; }

HealthValue Health_getHeartRate(void) { return s_heart_rate; }
