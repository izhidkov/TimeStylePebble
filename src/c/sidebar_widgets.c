...
void BatteryMeter_draw(GContext* ctx, int yPosition) {
  ...
  if (batteryImage) {
    gdraw_command_image_recolor(batteryImage, globalSettings.iconFillColor,
                                globalSettings.iconStrokeColor);
...
int DateWidget_getHeight() { return 58; }
...
void DateWidget_draw(GContext* ctx, int yPosition) {
  ...
  if (dateImage) {
    gdraw_command_image_recolor(dateImage, globalSettings.iconFillColor,
                                globalSettings.iconStrokeColor);
    gdraw_command_image_draw(
...
void Sleep_draw(GContext* ctx, int yPosition) {
  if (sleepImage) {
    gdraw_command_image_recolor(sleepImage, globalSettings.iconFillColor,
                                globalSettings.iconStrokeColor);
...
void Steps_draw(GContext* ctx, int yPosition) {
  if (stepsImage) {
    gdraw_command_image_recolor(stepsImage, globalSettings.iconFillColor,
                                globalSettings.iconStrokeColor);
  ...
  graphics_draw_text(
      ctx, steps_text, stepsSidebarFont,
      GRect(-2 + SidebarWidgets_xOffset, yPosition + yOffset, 35, 20),
      GTextOverflowModeFill, GTextAlignmentCenter, NULL);
}

int HeartRate_getHeight() { return 40; }

void HeartRate_draw(GContext* ctx, int yPosition) {
  if (heartImage) {
    gdraw_command_image_recolor(heartImage, globalSettings.iconFillColor,
                                globalSettings.iconStrokeColor);
  ...
  graphics_context_set_text_color(ctx, globalSettings.sidebarTextColor);
  graphics_draw_text(ctx, heart_rate_text, lgSidebarFont,
